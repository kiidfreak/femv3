...
from .serializers import (
    UserSerializer, BusinessSerializer, CategorySerializer,
...
class BusinessViewSet(viewsets.ModelViewSet):
    queryset = Business.objects.select_related('user', 'category').prefetch_related('products', 'services')
...
class ReviewViewSet(viewsets.ModelViewSet):
    ...
    permission_classes = [permissions.AllowAny]
    filter_backends = [DjangoFilterBackend]
    filterset_fields = ['business_id', 'product_id', 'service_id']
    ...
    def perform_create(self, serializer):
        ...
        if product and product.business != business:
            raise ValidationError({'error': 'Product does not belong to the specified business'})
        
        if service and service.business != business:
            raise ValidationError({'error': 'Service does not belong to the specified business'})

        if business.user == user:
             raise ValidationError({'error': 'You cannot review your own business, product, or service'})
